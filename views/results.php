...
<h3><?php echo count($defineds) . ' '; ?>defined match<?php if (count($defineds) != 1) { ?>es<?php } ?>:</h3>
...
//echo '<pre>';var_dump($sorted);echo '</pre>';

?>
	<li class="wordsList"><h4 id="wordHeader<?php echo $o; ?>"><span id="arrow<?php echo $o; ?>" class="arrowInactive"></span><?php echo $defined->word; ?></h4>
    <ul class="paddingFixInactive" id="definition<?php echo $o; ?>">
    	<?php foreach ($sorteds as $key=>$sorted) { ?>
      	<?php if (!empty($sorted)) { ?>
        	<li class="definitionList"><span class="pOS"><?php echo $posAbb[$key]; ?></span>
          	<ol class="definitions">
            	<?php foreach ($sorted as $def) { ?>
              	<li class="definitionItem"><?php echo $def; ?></li>
              <?php } ?>
            </ol>
          </li>
        <?php }?>
...
<h3><?php echo count($undefineds) . ' ';?>undefined match<?php if (count($undefineds) != 1) { ?>es<?php } ?>:</h3>
<ul>
<?php foreach ($undefineds as $undefined) { ?>
<li class="unwordsList"><a href="http://www.google.com/search?q=<?php echo str_replace(" ", "+", $undefined); ?>+definition" target="_blank"><h4><?php echo $undefined; ?><span class="googleSearch">search on google</span></h4></a> 	
</li>
<?php } ?>
</ul>
<?php } ?>
...
for (var i=0; i<8; i++) {
	//var test = i+1;
	(function() {
		resultsArray[i] = document.getElementById('arrow' + (i+1));
		resultsArray[i].addEventListener('click', function(){ alert(i);}, false);
	}());
}
